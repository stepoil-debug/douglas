    ...
    for table in tables:
        ...
        while i < len(rows):
            ...
            if opponent is None:
                ...
                continue
            a = _player(cell)
            b = _player(opponent.find("td", class_="t-name"))
            sa = _sets_won(row)
            sb = _sets_won(opponent)
...
def collect(target: date) -> tuple[list[dict[str, Any]], str]:
    ...
    for base in BASE_URLS:
        try:
            response = requests.get(f"{base}/results/", params=params, headers=headers, timeout=35)
...
def main() -> int:
    ...
    try:
        ...
        print(f"[TQE] Results collector unavailable: {exc}", file=sys.stderr, flush=True)
        return 42
